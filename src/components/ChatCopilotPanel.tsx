        ...
        <aside className="card h-full min-h-0 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    <Bot className="w-5 h-5 text-trading-accent-cyan" />
                    <h2 className="text-lg font-semibold text-trading-text-primary">智能对话</h2>
                </div>
                {streaming && (
            ...
            <div className="text-xs text-trading-text-muted mb-3 flex items-center gap-1">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                {PRESET_PROMPTS.map((prompt) => (
                    <button
                        ...
                        className="text-xs px-2.5 py-1 rounded-md border border-trading-border text-trading-text-secondary hover:border-trading-accent-blue/40 hover:text-trading-text-primary"
                    >
                        {prompt}
                    </button>
                ))}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-1">
                    ...
                    <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div
                            className={`max-w-[92%] rounded-xl px-3 py-2 text-sm leading-relaxed ${msg.role === 'user'
                                    ? 'bg-trading-accent-blue/15 border border-trading-accent-blue/35 text-trading-text-primary'
                                    : 'bg-trading-bg-tertiary border border-trading-border text-trading-text-secondary'
                                }`}
                        >
                            {msg.content}
                        </div>
                ...
                <div className="pt-2 border-t border-trading-border/80">
                    <p className="text-xs text-trading-text-muted mb-2">思考流程</p>
                    <div className="space-y-1.5">
                        {timelineLogs.slice(-120).map((log) => (
                            <div key={log.id} className="text-xs text-trading-text-secondary bg-trading-bg-primary/40 border border-trading-border/60 rounded-md px-2 py-1">
                                <span className="text-trading-text-muted mr-2">{new Date(log.timestamp).toLocaleTimeString()}</span>
                                {log.agent && <span className="text-trading-accent-cyan mr-2">{log.agent}</span>}
